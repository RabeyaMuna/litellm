...
def pre_process_non_default_params(
    ...
    if custom_llm_provider is not None and custom_llm_provider in [provider.value for provider in LlmProviders]:
        # Safely resolve provider enum to avoid ValueError/AttributeError when
        # the provider string does not map to a defined LlmProviders member.
        try:
            provider_enum = LlmProviders(custom_llm_provider)
        except Exception:
            provider_enum = None
        if provider_enum is not None:
            provider_config = ProviderConfigManager.get_provider_chat_config(model=model, provider=provider_enum)
...
def get_provider_info(model: str, custom_llm_provider: Optional[str]) -> Optional[ProviderSpecificModelInfo]:
    ...
    if custom_llm_provider and custom_llm_provider in LlmProvidersSet:
        # Safely resolve provider enum before requesting provider model info
        try:
            provider_enum = LlmProviders(custom_llm_provider)
        except Exception:
            provider_enum = None
        if provider_enum is not None:
            provider_config = ProviderConfigManager.get_provider_model_info(model=model, provider=provider_enum)
...
def get_provider_fields(custom_llm_provider: str) -> List[ProviderField]:
    ...
    if custom_llm_provider == "databricks":
        ...
        # Provide DigitalOcean required params if the config exists
        return litellm.DigitalOceanConfig().get_required_params()
...
def validate_environment(  # noqa: PLR0915
    ...
    if custom_llm_provider:
        if custom_llm_provider == "openai":
            ...
            # DigitalOcean provider environment validation
            if "DIGITALOCEAN_API_KEY" in os.environ:
                keys_in_environment = True
            else:
                missing_keys.append("DIGITALOCEAN_API_KEY")
    else:
...
def get_valid_models(
    ...
    try:
        ...
        for provider in valid_providers:
            # Safely resolve provider enum; if it cannot be resolved, skip
...
class ProviderConfigManager:
    ...
    def get_provider_chat_config(  # noqa: PLR0915
        ...
        # Use safe comparisons against the resolved provider enum's value
        # rather than referencing LlmProviders.DIGITALOCEAN directly which
        # may not exist on the enum in some installs.
        elif getattr(provider, "value", None) == "digitalocean":
