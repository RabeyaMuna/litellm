...
async def acompletion(
    ...
    **kwargs,
    ...
    if mock_delay and should_run_mock_completion(
        mock_response=mock_response, mock_tool_calls=mock_tool_calls, mock_timeout=mock_timeout
    ):
    ...
    **kwargs,
    ...
    try:
        ...
        if custom_llm_provider is not None:
            provider_enum = None
            try:
                if custom_llm_provider in [provider.value for provider in LlmProviders]:
        ...
        # Preserve attribute errors (e.g., missing enum members) so they aren't masked by a generic mapping
...
async def aembedding(*args, **kwargs) -> EmbeddingResponse:
    ...
    try:
        ...
        # Preserve attribute errors (such as missing enum members) so they surface clearly
        if isinstance(e, AttributeError):
            raise
        custom_llm_provider = custom_llm_provider or "openai"
        raise exception_type(
...
def transcription(
    ...
    **kwargs,
    ...
    try:
        provider_enum = None
        if custom_llm_provider is not None and custom_llm_provider in [p.value for p in LlmProviders]:
            try:
                provider_enum = LlmProviders(custom_llm_provider)
            except (ValueError, AttributeError):
                provider_enum = None
