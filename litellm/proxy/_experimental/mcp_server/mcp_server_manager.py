...
def _deserialize_json_dict(data: Any) -> dict[str, str] | None:
    """
...
class MCPServerManager:
    ...
    def _register_openapi_tools(self, spec_path: str, server: MCPServer, base_url: str):
        ...
        try:
            ...
            verbose_logger.error(
                f"Failed to register OpenAPI tools for server {server.name}: {e!s}"
            )
            raise e
    ...
    async def get_allowed_mcp_servers(
        self, user_api_key_auth: UserAPIKeyAuth | None = None
    ) -> list[str]:
        """
    ...
    async def get_tools_for_server(self, server_id: str) -> list[MCPTool]:
        ...
        try:
            ...
            verbose_logger.warning(
                f"Failed to get tools from server {server_id}: {e!s}"
            )
            return []
    ...
    async def _get_tools_from_server(
        ...
        try:
            ...
            verbose_logger.warning(
                f"Failed to get tools from server {server.name}: {e!s}"
            )
            return []
        finally:
            if client:
    ...
    def _parse_www_authenticate_header(
        ...
        scopes: list[str] | None = [s for s in (scope_value.split() if scope_value else []) if s]
    ...
    def _extract_scopes(self, scopes_value: Any) -> list[str] | None:
    ...
    async def _fetch_tools_with_timeout(
        ...
        async def _list_tools_task():
            try:
                ...
                verbose_logger.warning(
                    f"Client operation failed for {server_name}: {e!s}"
                )
                return []
        try:
            ...
            verbose_logger.warning(
                f"Connection error while listing tools from {server_name}: {e!s}"
            )
            return []
        except Exception as e:
    ...
    async def _call_regular_mcp_tool(
        ...
        try:
            ...
            # Re-raise guardrail exceptions to properly fail the MCP call
            verbose_logger.error(
                f"Guardrail blocked MCP tool call during result check: {e!s}"
            )
            raise e
    ...
    def get_mcp_server_by_name(self, server_name: str) -> MCPServer | None:
        """
