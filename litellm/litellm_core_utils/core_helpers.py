...
def get_litellm_metadata_from_kwargs(kwargs: dict):
    ...
    This handles both the normal top-level `metadata` payload and the nested
    `litellm_params.metadata` / `litellm_params.litellm_metadata` payloads used
    by router and proxy calls.
    ...
    if isinstance(litellm_params, dict):
        param_metadata = litellm_params.get("metadata") or {}
        if isinstance(param_metadata, dict):
            metadata = {**metadata, **param_metadata}

        if isinstance(litellm_metadata, dict):
            litellm_metadata = {**litellm_metadata, **param_metadata}
