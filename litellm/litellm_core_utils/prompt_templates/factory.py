...
# Literal may not be available in older typing implementations; fall back to typing_extensions
try:
...
class BedrockConverseMessagesProcessor:
    ...
    def _maybe_add_cache_point(
        ...
        if _cache_point_block is not None:
            # _cache_point_block may be a SystemContentBlock variant; cast to BedrockContentBlock
        ...
        while msg_i < len(messages):
            ...
            while msg_i < len(messages) and messages[msg_i]["role"] == "user":
                ...
                if isinstance(message_block["content"], list):
                    ...
                    BedrockConverseMessagesProcessor._maybe_add_cache_point(user_content, message_block)
            ...
            while msg_i < len(messages) and messages[msg_i]["role"] == "tool":
                ...
                BedrockConverseMessagesProcessor._maybe_add_cache_point(tool_content, messages[msg_i])
                msg_i += 1
            if tool_content:
            ...
            while msg_i < len(messages) and messages[msg_i]["role"] == "assistant":
                ...
                if _assistant_content is not None and isinstance(_assistant_content, list):
                    ...
                    BedrockConverseMessagesProcessor._maybe_add_cache_point(assistant_content, assistant_message_block)
