...
def _process_image_response(response: Response, url: str) -> str:
    if response.status_code != 200:
        # Handle non-200 responses gracefully: log a warning and return the original URL
        print(f"Warning: Unable to fetch image from URL. Status code: {response.status_code}, url={url}")
        return url
